...
describe("words", () => {
  it("returns an error if called without arguments", () => {
    expect(() => words()).toThrow(TypeError("First argument needs to be a positive integer"));
  });

  it("returns an error if called with a string size argument", () => {
    expect(() => words("hello")).toThrow(TypeError("First argument needs to be a positive integer"));
  });

  it("returns an error if called with a negative integer size argument", () => {
    expect(() => words(-4)).toThrow("First argument needs to be a positive integer");
  });
  ...
  it.each([
    ...
    { size: 5, format: false },
    { size: 5, format: 12434 },
  ])("returns an error if called with invalid format argument '$format'", ({ size, format }) => {
    expect(() => words(size, format)).toThrow(Error(`Invalid format '${format}' picked`));
  });
  ...
  it.each([{ size: 2 }, { size: 5 }, { size: 10 }, { size: 25 }, { size: 100 }, { size: 1000 }])(
  ...
  ])("words($size, $format) returns an array of size $size of uppecase words", ({ size, format }) => {
  ...
  ])("words($size, $format) returns an array of size $size of capitalized words", ({ size, format }) => {
    const actual = words(size, format);
    expect(typeof actual).toBe("object");
    expect(actual.length).toBe(size);
    actual.forEach(w => {
